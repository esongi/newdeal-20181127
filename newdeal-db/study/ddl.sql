...
-- 강의실
DROP TABLE IF EXISTS nd_clas_room RESTRICT;
...
-- 강의실
ALTER TABLE nd_clas_room
  ADD CONSTRAINT PK_nd_clas_room -- 강의실 기본키
    PRIMARY KEY (
      cmo -- 강의실번호
    );

-- 강의실 유니크 인덱스
CREATE UNIQUE INDEX UIX_nd_clas_room
  ON nd_clas_room ( -- 강의실
    loc ASC,  -- 교육센터
    room ASC  -- 강의실
  );
...
-- 수업
ALTER TABLE nd_lesn
  ADD CONSTRAINT FK_nd_clas_room_TO_nd_lesn -- 강의실 -> 수업
    FOREIGN KEY (
      cmo -- 강의실번호
    )
    REFERENCES nd_clas_room ( -- 강의실
      cmo -- 강의실번호
    );
